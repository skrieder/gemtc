...
# date                    : 2014/05/03 15:33:04
# Turbine version         : 0.3.0
# Input filename          : /home/skrieder/gemtc/examples/swift/simple_cuda/test-sleep-1.swift
# Output filename         : /home/skrieder/gemtc/examples/swift/simple_cuda
...
# stc.ic.output-file            : 
# stc.input_filename            : test-sleep-1.swift
# stc.log.file                  : 
# stc.log.trace                 : false
...
proc swift:main {  } {
    ...
    adlb::spawn 0 "-1 main-call_sync-sleep_wrapper ${stack} ${optv:sleepTime}"
    turbine::reset_priority
}


proc main-call_sync-sleep_wrapper { stack optv:sleepTime } {
    # Var: $int optv:sum VALUE_OF [int:sum]
    set optv:sum [ sleep_wrapper ${optv:sleepTime} ]
}


proc f:sleep_wrapper { stack u:sum u:i1 dr:location } {
    turbine::c::log "enter function: sleep_wrapper"
    turbine::read_refcount_incr ${u:i1} 1
    turbine::rule [ list ${u:i1} ] "sleep_wrapper-argwait ${stack} ${u:sum} ${u:i1}" target ${dr:location} type ${::turbine::WORK}
}


proc sleep_wrapper-argwait { stack u:sum u:i1 } {
    # Var: $int v:i1 VALUE_OF [int:i1]
    # Var: $int v:sum VALUE_OF [int:sum]
    set v:i1 [ turbine::retrieve_integer ${u:i1} CACHED 1 ]
    set v:sum [ sleep_wrapper ${v:i1} ]
