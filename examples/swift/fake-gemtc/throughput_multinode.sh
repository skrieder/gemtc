...
echo "Job#, NODES, PPN, Tasks, Sleep Time, GPU Workers, Output Dir" >> logs/throughput_log_$launch_time.txt
...
# Write to the log file
	echo $counter"," $NODES"," $PPN"," $bound"," $sleep_time"," $gpu_workers"," $output_dir >> logs/throughput_log_$launch_time.txt

echo 'grep "ADLB Total Elapsed Time:" '$output_dir'/output.*.*|awk -F: ''{print $2}'' '>> logs/scrape_throughput_log_$launch_time.sh
