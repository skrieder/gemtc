...
# date                    : 2013/11/02 20:22:07
# Turbine version         : 0.3.0
# Input filename          : /home/skrieder/exm/apps/fake-gemtc/002_md.swift
...
# stc.ic.output-file            : 
# stc.input_filename            : 002_md.swift
# stc.log.file                  : 
# stc.log.trace                 : false
...
proc swift:main {  } {
    ...
    lassign [ adlb::multicreate [ list integer 2 ] [ list integer 1 ] [ list container integer ref 2 1 ] [ list container integer ref 1 1 ] [ list blob 1 ] [ list blob 1 ] [ list blob 0 ] [ list blob 0 ] ] u:np u:n u:a_position_array u:b_position_array u:b u:c u:a_result u:b_result
    turbine::c::log "allocated u:np=<${u:np}> u:n=<${u:n}> u:a_position_array=<${u:a_position_array}> u:b_position_array=<${u:b_position_array}> u:b=<${u:b}>"
    turbine::c::log "allocated u:c=<${u:c}> u:a_result=<${u:a_result}> u:b_result=<${u:b_result}>"
    ...
    # Swift l.25: assigning expression to a_position_array
    ...
    # Swift l.26: assigning expression to b_position_array
    # Swift l.35: assigning expression to b
    turbine::blob_from_floats [ list ${u:b} ] [ list ${u:a_position_array} ]
    # Swift l.36: assigning expression to c
    turbine::blob_from_floats [ list ${u:c} ] [ list ${u:b_position_array} ]
    # Swift l.45: assigning expression to a_result
    gemtc::gemtc_mdproxy [ list ${u:a_result} ] [ list ${u:np} ${u:b} ]
    # Swift l.46: assigning expression to b_result
    gemtc::gemtc_mdproxy [ list ${u:b_result} ] [ list ${u:np} ${u:c} ]
    # Swift l.48: assigning expression to a_result_array
    # Swift l.49: assigning expression to b_result_array
    turbine::rule [ list ${u:a_position_array} ] "main-arrcopy-wait0 ${stack} ${u:a_position_array} ${u:b_position_array}"
}


proc main-arrcopy-wait0 { stack u:a_position_array u:b_position_array } {
    set tcltmp:contents [ adlb::enumerate ${u:a_position_array} dict all 0 ]
    dict for {v:0 a:0} ${tcltmp:contents} {
        turbine::read_refcount_incr ${a:0} 1
        turbine::container_insert ${u:b_position_array} ${v:0} ${a:0} ref 0
    }
