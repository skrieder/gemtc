...
proc swift:main {  } {
    ...
    # Var: $int optv:num_elements VALUE_OF [int:num_elements]
    # Var: $string optv:__t2 VALUE_OF [string:__t2]
    # Var: $int optv:num_threads VALUE_OF [int:num_threads]
    # Swift l.9: assigning expression to num_elements
    set optv:__t0 [ turbine::argv_get_impl "num_elements" ]
    set optv:num_elements [ turbine::check_str_int ${optv:__t0} ]
    # Swift l.10: assigning expression to num_threads
    set optv:__t2 [ turbine::argv_get_impl "num_threads" ]
    set optv:num_threads [ turbine::check_str_int ${optv:__t2} ]
    # Swift l.11: assigning expression to sum
    set tcltmp:prio [ turbine::get_priority ]
    turbine::set_priority ${tcltmp:prio}
    adlb::spawn 0 "-1 main-call_sync-c_saxpy ${stack} ${optv:num_threads} ${optv:num_elements}"
    turbine::reset_priority
}


proc main-call_sync-c_saxpy { stack optv:num_threads optv:num_elements } {
    # Var: $int optv:sum VALUE_OF [int:sum]
    set optv:sum [ c_saxpy ${optv:num_elements} ${optv:num_threads} ]
}


proc f:c_saxpy { stack u:sum u:i1 u:i2 dr:location } {
    turbine::c::log "enter function: c_saxpy"
    turbine::read_refcount_incr ${u:i1} 1
    turbine::read_refcount_incr ${u:i2} 1
    turbine::rule [ list ${u:i1} ${u:i2} ] "c_saxpy-argwait ${stack} ${u:sum} ${u:i2} ${u:i1}" target ${dr:location} type ${::turbine::WORK}
}


proc c_saxpy-argwait { stack u:sum u:i2 u:i1 } {
